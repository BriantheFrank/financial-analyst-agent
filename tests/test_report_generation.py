...
def test_chart_outputs_created(tmp_path: Path):
    ...
    assert (outdir / "10_data_coverage.png").exists()
...
def test_missing_segment_graceful_skip(tmp_path: Path):
    ...
    assert not results["03_revenue_by_segment"].generated
    assert "unavailable" in results["03_revenue_by_segment"].reason.lower()
